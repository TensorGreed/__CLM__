...
| R1-E04: Certificate Inventory Core | Backend Done | Backend for R1-E04-S01 through R1-E04-S06: PEM-only certificate import, certificate/version records, parsed metadata, fingerprint deduplication, source observations, tenant-scoped search/detail APIs, editable tags, typed custom metadata, status history, RBAC, and audit events. | Backend tests, migration tests, docs lint, Compose config, Compose rebuild/startup, API health, API root, OpenAPI JSON, protected certificate API 401, new OpenAPI path checks, and UI probe passed. | Frontend inventory screens move to R1-E05. Private keys, issuance, renewal, destinations, plugins, and MCP remain out of scope. |
| R1-E05: Frontend Operational Shell | In Progress | R1-E05-S01 through R1-E05-S03: role-aware shell, top search, tenant selector, user/access menu, route guards, inventory table, filters, sorting, pagination, saved columns, bulk selection, and certificate detail sections for summary, versions, chain, sources, metadata, status history, automation placeholders, and audit timeline. | Frontend lint, tests, build, backend tests, docs lint, Compose config, Compose UI recreation, API health, API root, OpenAPI JSON, protected certificate API 401, Vite proxy protected certificate API 401, UI root, and UI inventory route probes passed. | Settings pages and global cross-resource search remain in R1-E05-S04 and R1-E05-S05. Private keys, issuance, renewal, destinations, plugins, and MCP remain out of scope. |
...
1. Implement R1-E05-S04 and R1-E05-S05 for frontend settings pages and global cross-resource search.
2. Add the minimal backend search endpoint needed by R1-E05-S05 if the existing certificate list API is not enough.
3. Move to R2-E01 only after settings and search are stable, because private key handling has a much higher security bar.
