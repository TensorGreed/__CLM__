...
| R1-E01: Repository And Build Foundation | Done | Spring Boot backend skeleton, React TypeScript frontend shell, local Compose stack, baseline CI, dependency automation, docs tooling, and developer/operator docs. | Backend tests, frontend lint/tests/build, docs lint, Compose config, Compose startup, API probes, and UI probes passed. | Local runtime validation is covered by the current Compose smoke checks. |
...
| R2-E01: Certificate Import And Key Handling | In Progress | Backend guardrail and reference slices: public certificate import remains private-key rejecting, the guarded private-key import path validates RSA/EC key matches, database private key persistence stays disabled and rejected, key-handling config placeholders are present, approved external key references can be recorded when explicitly enabled, public key size metadata is parsed, validation/policy/reference events are audited, and PEM/key redaction tests are covered. | Backend tests, focused key-handling integration tests, frontend lint/tests/build, docs lint, Compose config, Compose rebuild/startup, API health, API root, OpenAPI JSON, protected certificate APIs, private-key import guardrail API, and UI probes passed. | Real secret storage provider integrations, key export controls, bulk import, deeper chain validation, issuance, renewal, destinations, plugins, and MCP remain. |
...
1. Finish R2-E01-S01 by adding an internal key storage provider abstraction that supports external secret references first, with database private key persistence still disabled.
2. Implement R2-E01-S02 bulk import dry-run and validation reports on top of the same public/private-key validation services.
3. Then implement R2-E01-S03 private key export controls as disabled-by-default, permission-gated, reason-gated, and fully audited policy scaffolding.
